...
def train(num_epochs, interval, model_save, model_log):
    ...
    for epoch in range(1, num_epochs + 1):    
        ...
        for sample in val_loader:
            ...
            result = model(A)
            val_loss += F.l1_loss(result, B)

        val_loss /= len(val_dataset)
        print('\nEpoch: {} Average validation loss: {:.4f} \n'.format(epoch, val_loss))
...
def val(model_log):
    ...
    with torch.no_grad():
        for sample in val_loader:
            ...
            val_ssim += ssim(Y.squeeze(0).numpy(), output.squeeze(0).numpy())
...
if __name__ == '__main__':
    ...
    train(50, 50, 'unet_train02.pt', model_log=None)
